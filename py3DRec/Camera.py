...
class myCamera(object):
	...
	def __init__(self,k):
		...
		PARAMETERS:
		@k : (3x3) matrix (numpy.mat) with the camera intrinsics
		"""
		self.P = None	  # 
	...
	#==========================================================
	@staticmethod
	def CalibrateUsingImages(format_name, matrix_name):
		"""
		CalibrateUsingImages(format_name, matrix_name)
		static member
		reads the image data generated by GenerateImageDataset 

		PARAMETERS:
		@formatname: string. ex. 'apic*.jpg'
		@matrix_name: string. ex. 'k_cam_hp.dat'

		OUTPUT:
		returns a (3x3) matrix (numpy.mat) with the camera intrinsics
		"""
		# termination criteria
		criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)

		#number of cols and rows in the chesboard image
		ix = 9
		iy = 6
		...
		f = open(matrix_name, 'w')
		#repr = python function that represents an object in a string format
	...
	def GenerateImageDataset(tagname, delay_time, time_interval, nsamples):
		"""
		GenerateImageDataset(tagname, delay_time, time_interval, nsamples)
		static member
		generates a image dataset. the user must have a chessboar in front the webcam and take pictures from different views.

		PARAMETERS:
		@tagname: string. tag before the picture name. it is saved in the current folder ex. 'apic'
		@delay_time: integer. time before start the capture, in ms. 
		@time_interval: integer. time between each capture, in ms.
		@nsamples: integer. number of sampes

		OUTPUT:
		returns nothing, however it generates nsamples saved in the current folder.
		"""
	...
	def projectiveMatrix(self,t,r):
		"""
		projectiveMatrix(self,t,r)
		Creates a projective matrix K * I * [R | t]

		PARAMETERS:
		@t: R3 vector representing the translational movement
		@R: R3 or R(3,3) matrix representing the rotational movement


		OUTPUT:
		stores the projective matrix in the variable self.P
		"""
		t = np.mat(t)
